...
BEGIN
  ...
  employee_name_value := COALESCE(NULLIF(BTRIM(NEW.real_name), ''), NULLIF(BTRIM(NEW.display_name), ''), '名称未設定');
  ...
  IF employee_id_value IS NULL THEN
    SELECT COUNT(*), (ARRAY_AGG(employee.id ORDER BY employee.created_at, employee.id))[1]
    INTO candidate_count, candidate_employee_id
    FROM public.gw_payroll_employees employee
    WHERE employee.user_id IS NULL
      AND employee.payroll_status <> 'retired'
      AND REGEXP_REPLACE(COALESCE(employee.real_name, employee.display_name), '[[:space:]　]', '', 'g')
  ...
  IF employee_id_value IS NULL THEN
    INSERT INTO public.gw_payroll_employees (
      user_id, display_name, real_name, department, default_workplace_id, source_key, raw_payload
    )
    VALUES (
      NEW.id,
      employee_name_value,
      ...
      JSONB_BUILD_OBJECT('source', 'gw_users', 'line_user_id', NEW.line_user_id)
    )
    RETURNING id INTO employee_id_value;
  END IF;
...
UPDATE public.gw_users SET status = status WHERE status = 'approved';
