...
assert.match(result.content, /取得workerがブラウザアクセス制限を報告/)
assert.match(result.content, /実際の原因はログと画面で確認が必要/)
assert.match(result.content, /「実行」を押します/)
assert.doesNotMatch(result.content, /「手動実行」/)
