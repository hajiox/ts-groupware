...
const redirectUri = process.env.GOOGLE_GMAIL_REDIRECT_URI ||
  ...
  `http://localhost:${process.env.GOOGLE_GMAIL_OAUTH_PORT || process.env.GOOGLE_DRIVE_OAUTH_PORT || 53682}/oauth2callback`
...
if (codeArgIndex >= 0) {
  ...
  if (!code) {
    console.error('Usage: node scripts/google-gmail-oauth-token.mjs --code <authorization_code>')
    process.exit(1)
  }
...
const authUrl = oauth2Client.generateAuthUrl({
  ...
  login_hint: process.env.GOOGLE_GMAIL_LOGIN_HINT || process.env.GOOGLE_DRIVE_LOGIN_HINT,
})
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    console.log(tokens.refresh_token)
    console.log('\nAdd this value to Vercel Production as GOOGLE_GMAIL_REFRESH_TOKEN.')

    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' })
    res.end('<h1>Authorization complete</h1><p>You can close this tab. The refresh token is printed in the terminal.</p>')
  } catch (error) {
...
server.listen(port, () => {
  console.log(`Open this URL and authorize Gmail send access:\n\n${authUrl}\n`)
