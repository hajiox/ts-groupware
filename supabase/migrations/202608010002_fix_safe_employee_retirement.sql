-- Qualify the push subscription column because the table-returning function
-- also exposes a user_id output variable in PL/pgSQL.
...
begin
  if p_resigned_date is null then
    raise exception '退職日を入力してください';
  end if;

  select employee.user_id, employee.hire_date
    into target_user_id, target_hire_date
  ...
  set payroll_status = 'retired',
      resigned_date = p_resigned_date,
  ...
  if target_user_id is not null then
    ...
    delete from public.gw_push_subscriptions subscriptions
    where subscriptions.user_id = target_user_id;
  end if;

  return query
  select employee.id, employee.user_id, employee.resigned_date, employee.payroll_status
  from public.gw_payroll_employees employee
  where employee.id = p_employee_id;
end;
$$;
