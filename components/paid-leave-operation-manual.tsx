  ...
  return (
    <article className="manual-print-root attendance-manual paid-leave-manual">
      ...
      <section className="manual-section">
        ...
        <p className="manual-note">
          全休は、確定シフトが休みの日でも申請できます。送信時点ではシフトを変更せず、
          管理者の承認後に休みから有給へ変更し、残日数と労務用の打刻へ反映します。
          半休は勤務時間が必要なため、確定シフトに勤務枠がある日に申請してください。
        </p>
      </section>
      ...
      <section className="manual-section">
        <h2>6. 半休・遅刻・早退・打刻忘れ</h2>
        <ul>
          <li>有給（全休）は1日、有給（半休）は0.5日として残日数から差し引きます。</li>
          <li>半休の日に勤務した時間は、専用タイムレコーダーで通常どおり出勤・退勤を打刻します。</li>
          <li>2026年8月25日以降、正社員は確定シフトに対する遅刻・早退、または打刻なしがあると、TSGに勤怠確認が表示されます。</li>
          <li>勤怠確認では「欠勤扱い」「有給（半休・0.5日使用）」「管理者に連絡」を選びます。打刻忘れの場合は打刻忘れも選択できます。</li>
          <li>遅出・退勤時刻変更を管理者が承認した日は、遅刻・早退の回数へ含めません。</li>
          <li>時間制スタッフは実際の勤務時間で給与計算し、遅刻・早退の回数と時間を記録します。打刻が不足している日は本人確認が表示されます。</li>
      ...
      <section className="manual-section">
        ...
        <ul>
          ...
          <li>本人が操作できない場合は、所属管理者が有給・欠勤管理から代理設定できます。</li>
